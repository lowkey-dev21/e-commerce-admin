...
const ROLES = [
  {
    ...
    color: "#E6F7F7",
    textColor: "#4AB7B6",
    permissions: ["dashboard", "products", "orders", "customers", "transactions", "coupons", "categories", "brands", "admin"],
  },
  {
    id: 2,
    ...
    color: "#FFF3E4",
    textColor: "#F4A94E",
    permissions: ["dashboard", "products", "categories", "brands"],
  },
  {
    id: 3,
    name: "Order Manager",
    ...
    color: "#F0F4FF",
    textColor: "#3B82F6",
    permissions: ["dashboard", "orders", "customers", "transactions"],
  },
  {
    id: 4,
    name: "Support Agent",
    description: "View orders and handle customer queries",
    users: 8,
    color: "#FFF0F0",
    textColor: "#EF4444",
          ...
          <div key={role.id} className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm hover:shadow-md transition">
            <div className="flex items-start justify-between mb-3">
              ...
              <div className="flex gap-2">
                <button className="text-xs text-[#4AB7B6] bg-[#E6F7F7] px-2.5 py-1 rounded-lg font-semibold hover:bg-[#4AB7B6]/20 transition">Edit</button>
                {role.id !== 1 && <button className="text-xs text-red-500 bg-red-50 px-2.5 py-1 rounded-lg font-semibold hover:bg-red-100 transition">Delete</button>}
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {role.permissions.map((p) => (
                <span key={p} className="text-[10px] font-medium px-2 py-0.5 bg-slate-100 text-slate-600 rounded-full capitalize">
                  {p}
                </span>
              ))}
            </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-base font-bold text-slate-800">Create New Role</h3>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" placeholder="e.g. Content Editor" className="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#4AB7B6]/30 focus:border-[#4AB7B6]" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1.5">Description</label>
                <input type="text" placeholder="Brief description..." className="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#4AB7B6]/30 focus:border-[#4AB7B6]" />
              </div>
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {ALL_PERMISSIONS.map((p) => (
                    <label key={p} className="flex items-center gap-2 cursor-pointer">
                      <input type="checkbox" className="w-3.5 h-3.5 accent-[#4AB7B6]" />
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={() => setShowForm(false)} className="flex-1 py-2 bg-[#4AB7B6] text-white rounded-xl text-sm font-semibold hover:bg-[#3aa3a2] transition">Create Role</button>
            </div>
          </div>
        </div>
      )}
