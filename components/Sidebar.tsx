...
const MAIN_NAV: NavItem[] = [
  ...
  { href: "/orders", label: "Order Management", icon: <OrderIcon /> },
  ...
  { href: "/brands", label: "Brand", icon: <StoreIcon /> },
];
...
function NavLink({ item, pathname }: { item: NavItem; pathname: string }) {
  const isActive = item.exact ? pathname === item.href : pathname.startsWith(item.href) && (item.href !== "/" || pathname === "/");
  return (
    <Link
      href={item.href}
      className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${
        isActive
          ? "bg-[#4AB7B6] text-white shadow-sm"
  ...
  return (
    <aside className="w-56 bg-white border-r border-slate-200 flex flex-col shadow-sm shrink-0">
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-0">
        <NavSection title="Main menu" items={MAIN_NAV} pathname={pathname} />
        <NavSection title="Product" items={PRODUCT_NAV} pathname={pathname} />
      ...
      <div className="p-3 border-t border-slate-100">
        <div className="flex items-center gap-3 p-2 rounded-xl hover:bg-slate-50 cursor-pointer transition">
          <div className="w-8 h-8 rounded-full bg-[#4AB7B6] flex items-center justify-center text-white text-xs font-bold shrink-0">
            A
          </div>
